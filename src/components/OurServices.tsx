...
//           <a href="#contact">
//             <button className="mt-3 inline-flex items-center justify-center gap-1.5 rounded-lg px-4 py-2 text-xs font-semibold text-white transition-all active:scale-95 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 lg:gap-1.5 lg:rounded-xl lg:px-6 lg:py-3 lg:text-sm bg-[#F05B2D]">
//               <svg
//                 width="16"
//                 height="16"
//                 viewBox="0 0 16 16"
...
const services = [
  ...
  },
  {
    ...
    description:
      "Enterprise infrastructure that grows with you. No licenses, no equity grabs, no BS.",
    icon: "/service-icon5.webp",
    image: "/service-image5.webp",
  },
  {
    title: "Unlimited Scale Support",
    description:
      "World-class support, training & growth optimization tailored to your product.",
    icon: "/service-icon6.webp",
    image: "/service-image6.webp",
  },
];
...
const OurServices = () => {
  ...
  return (
    <div className="relative bg-[#0431C2] py-4 lg:py-10" id="OurServices">
      {/* top svg  */}
      <motion.div
        className="absolute  left-50 top-6 sm:left-10 sm:top-10"
        animate={{
      ...
      <div className="text-center mt-10">
        <button className="bg-[#CFB994] text-black text-sm font-bold py-2 px-4 rounded-full">
          Our Services
        </button>
        <h2 className="text-white text-4xl lg:text-5xl font-bold mt-6">
          White-Label Dominance
        </h2>
      </div>

      {/* Desktop view */}
      <div className="hidden lg:flex flex-wrap gap-y-8 max-w-screen-2xl mx-auto px-4 py-12">
        {services.map((service, index) => (
          <div
            key={index}
            className={`w-1/3 p-4 relative ${
              index % 2 === 0 ? "top-10" : "top-0"
            }`}
          >
            <div className="flex flex-col space-y-4 rounded-3xl border border-white border-opacity-70 bg-opacity-10 p-6 text-white bg-[#2246C8]">
              <Image
                src={service.icon}
                alt={service.title}
                width={71}
      ...
      {/* Mobile carousel */}
      <div className="lg:hidden max-w-md mx-auto px-4 py-12 relative">
        {/* Slider arrows */}

        <button
          onClick={prevSlide}
          className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-[#F05B2D] text-white text-2xl px-3 py-1 rounded-full z-10"
        ...
        <button
          onClick={nextSlide}
          className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-[#F05B2D] text-white text-2xl px-3 py-1 rounded-full z-10"
              ...
              <div key={index} className="min-w-full p-4">
                <div className="flex flex-col space-y-4 rounded-3xl border border-white border-opacity-70 bg-opacity-10 p-6 text-white bg-[#2246C8]">
                  ...
                  <Image
                    ...
                    width={300}
                    height={300}
                    className="h-[160px] w-full rounded-3xl object-cover"
                  />
                </div>
              </div>
            ))}
      ...
      {/* CTA Button */}
      <div className="flex justify-center mt-4 sm:mt-2 sm:mb-10">
        <a href="#contact">
          <button className="inline-flex items-center justify-center gap-1.5 rounded-xl px-6 py-3 text-sm font-semibold text-white bg-[#F05B2D]">
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
